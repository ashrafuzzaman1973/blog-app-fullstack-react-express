        ...
        <div className="min-h-screen bg-slate-100 py-6 px-4 flex flex-col items-center font-sans gap-6">
            ...
            <div className="w-full max-w-4xl bg-white shadow-md rounded-2xl p-2 flex items-center justify-between border border-slate-200">
                <div className="flex gap-2">
                    <button onClick={() => setView("feed")} className={`px-6 py-2 rounded-xl font-bold transition-all ${view === "feed" ? "bg-emerald-700 text-white shadow-lg" : "text-slate-600 hover:bg-slate-100"}`}>🏠 Home Feed</button>
                    {token && <button onClick={() => setView("chat")} className={`px-6 py-2 rounded-xl font-bold transition-all ${view === "chat" ? "bg-emerald-700 text-white shadow-lg" : "text-slate-600 hover:bg-slate-100"}`}>💬 Messages</button>}
                </div>
                {token ? (
                    <div className="flex items-center gap-4 pr-4">
                        <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{userEmail}</span>
                        <button onClick={onLogout} className="text-red-500 font-bold text-sm hover:underline">Logout</button>
                    </div>
                ) : <div className="pr-4 italic text-slate-400 text-xs">Login to access features</div>}
            </div>
            ...
            <div className="w-full max-w-4xl">
                {view === "feed" ? (
                    <div className="space-y-6">
                        {!token ? (
                            <div className="bg-white p-8 rounded-2xl shadow-xl border text-center max-w-md mx-auto">
                                <h2 className="text-2xl font-bold mb-4">{isLoginMode ? "Welcome Back" : "Create Account"}</h2>
                                <input className="w-full p-3 border rounded-xl mb-3 outline-none" placeholder="Email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} />
                                <input type="password" className="w-full p-3 border rounded-xl mb-3 outline-none" placeholder="Password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} />
                                <button onClick={handleAuthSubmit} className="w-full bg-emerald-700 text-white font-bold py-3 rounded-xl mb-2">{isLoginMode ? "Login" : "Register"}</button>
                                <p className="text-sm text-slate-500 cursor-pointer" onClick={() => setIsLoginMode(!isLoginMode)}>{isLoginMode ? "New here? Sign up" : "Have account? Login"}</p>
                            </div>
                        ) : (
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {title.length > 2 && (
                                        <button
                                            ...
                                            className="bg-purple-600 text-white px-4 rounded-xl text-xs font-bold hover:bg-purple-700 transition-all flex items-center gap-1 whitespace-nowrap shadow-md"
                                        >
                        ...
                        <div className="grid gap-6">
                            {loading ? <p className="text-center py-10 text-slate-400">Loading your feed...</p> : (
                                blogs.map((b) => (
                                    <article key={b._id} className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden hover:shadow-xl transition-all">
                                        ...
                                        <div className="p-6">
                                            <div className="flex justify-between items-start mb-3">
                                                ...
                                                <span className="text-[10px] uppercase font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">{b.email.split('@')[0]}</span>
                                            </div>
